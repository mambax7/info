...
switch ($op) {
	...
	case 'approved':
			...
			foreach ($infowait as $t => $tc) {
				$dellink = "<a href='admin_seiten.php?op=appdel&cat=" . $cat . '&id='
                           . $tc['info_id'] . "'><img src='" . $pathIcon16 . "/delete.png' title='" . _DELETE . "' alt='" . _DELETE . "'></a>";
				$editlink = "<a href='admin_seiten.php?op=appedit&cat=" . $cat . '&id='
                            . $tc['info_id'] . "'><img src='" . $pathIcon16 . "/edit.png' title='" . _EDIT . "' alt='" . _EDIT . "'></a>";
				$edittime = formatTimestamp($tc['edited_time'], 'l');
				$form->addElement(new XoopsFormLabel($editlink . ' | '
                                                     . $dellink . ' '
                                                     . $tc['title'], _INFO_LAST_EDITED . ': '
                                                                     . sprintf(_INFO_LAST_EDITEDTEXT, XoopsUserUtility::getUnameFromId($tc['edited_user'], 0, false), $edittime)));
			}
			$form->display();
			xoops_cp_footer();
		break;
    ...
    if ($id > 0) {
      ...
      xoops_cp_header();
			...
			$msg = _INFO_SETDELETE . '<br /><br />'
                   . sprintf(_INFO_INFODELETE_FRAGE, $content->getVar('title'));
      ...
      $id = $_POST['id'];
        ...
        if (intval($storyid) > 0) {
          $fpp = ($storyid==$fp) ? 1:0;
					if ($fpp==1) {
            $sql= 'UPDATE '
                  . $xoopsDB->prefix($xoopsModule->getVar('dirname')) . ' SET frontpage=0 WHERE frontpage>0';
	...
	case 'show':
		...
		$sql= 'SELECT cat_id,title FROM '
              . $xoopsDB->prefix($xoopsModule->getVar('dirname') . '_cat') . ' ORDER BY title ASC';
        $result=$xoopsDB->query($sql);
        $blist=array();
        if ($result) 		{
        ...
        $block_select->setextra('onchange="document.forms.'.$xoopsModule->getVar('dirname') . '_form_groupcat'
                                . '.submit()"');
        ...
        $group_select->setExtra('onchange="document.forms.' . $xoopsModule->getVar('dirname') . '_form_groupcat'
                                . '.submit()"');
		...
		foreach ( $info as $z => $tcontent)
		{
			...
			echo '</td><td>';
				$info_tree->makeMySelBox('title', 'blockid', $tcontent['parent_id'], 1, 'parent_id['
                                                                                        . $tcontent['info_id'] . ']', '', ' AND cat=' . $cat . ' AND info_id<>'
                                                                                                                          . $tcontent['info_id']);
			  echo '</td><td>';
				...
				if ($tcontent['link'] == 3) { //kategorie
					...
					echo "<a href='".XOOPS_URL . '/modules/'
                         . $xoopsModule->dirname() . '/index.php?content='
                         . $tcontent['info_id'] . "'>" . $title . '</a>';
				}
			echo '</td><td>';
			...
			echo "<td width=\"1%\" nowrap><select name='visible[".$tcontent['info_id']."]'><option value='0' ".$check1 . ' />'
                 . _NO . "</option><option value='1' " . $check2 . ' />'
                 . _YES . '</option></select></td>';
			echo "<td width=\"1%\" nowrap>&nbsp;";
			if ($tcontent['link'] != 3) {
				echo "<select name='submenu[".$tcontent['info_id']."]'><option value='0' ".$check5 . ' />'
                     . _NO . "</option><option value='1' " . $check7 . ' />'
                     . _YES . '</option></select>';
			} else {
			...
			if ($tcontent['link'] == 0 || $tcontent['link'] == 4 || $tcontent['link'] == 5) {
				echo "<select name='nocomments[".$tcontent['info_id']."]'><option value='1' ".$check4 . ' />'
                     . _NO . "</option><option value='0' " . $check5 . ' />'
                     . _YES . '</option></select>';
			} else {
...
{
	global $info_tree;
    $infolist = $info_tree->getAllChild(0, 'blockid', array(), ' AND cat='
                                                               . $cat . ' AND info_id<>'
                                                               . $aktuell);
